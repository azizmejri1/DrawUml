...
export class DiagramService {
    ...
    async generateUml(diagramDto : DiagramDto){
        const {description, type} = diagramDto;
        const prompt = "You are a PlantUML generator. Based on the following system description, generate the appropriate UML diagram using PlantUML syntax. The diagram type is:"+type+".System Description:"+description+"Output only valid PlantUML code between @startuml and @enduml. Do not add any explanations or markdown syntax."
        ...
        .then(response => {
          return this.addDiagram(response.data.choices[0].message.content);
        })
        .catch(error => {
